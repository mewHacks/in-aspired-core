// Toggle button to save/unsave a course to the user's bookmarks
...
const SaveCourseButton: React.FC<SaveCourseButtonProps> = ({
  courseId,
  courseName,
  variant = 'icon',
  showCount = false,
  ...
  const saved = isControlled ? !!propSaved : isSaved(courseId);
  ...
  const handleToggleSave = async (e?: React.MouseEvent) => {
    ...
    try {
      if (isControlled && onSaveChange) {
        ...
        const newState = await toggleSave(courseId);

        if (newState) {
          toast.success(
            <span>
              {t('ui.toasts.savedItem', 'Saved "{{name}}".', { name: courseName })}{' '}
              <button
                onClick={() => navigate('/saved?tab=courses')}
                className="underline font-semibold hover:text-indigo-700 dark:hover:text-indigo-400"
              >
                {t('ui.toasts.viewSavedCourses', 'View Saved Courses')}
              </button>
            </span>
          );
        } else {
          toast.info(t('ui.toasts.removedItem', 'Removed "{{name}}" from saved', { name: courseName }));
        }
      }
    } catch (error: any) {
  ...
  return variant === 'button' ? (
    ...
    >
      ...
      {showCount && <span className="text-sm font-medium">({savedCount})</span>}
    </button>
  ) : (
    <button
      onClick={handleToggleSave}
